...
- [`reference-backend/README.md`](https://github.com/snowdamiz/mesh-lang/blob/main/reference-backend/README.md) — deepest runbook for build, runtime, staged deploy, and smoke verification
...
These are the primary repo-level commands behind the backend proof story:
...
Deployment and recovery claims are backed by these named proofs:
...
- **Worker recovery and health visibility:** the ignored restart proofs, `GET /health`, and `GET /jobs/:id`
- **Documentation drift:** `bash reference-backend/scripts/verify-production-proof-surface.sh`

For the full command list and the runtime/deploy environment contract, continue to [`reference-backend/README.md`](https://github.com/snowdamiz/mesh-lang/blob/main/reference-backend/README.md).
