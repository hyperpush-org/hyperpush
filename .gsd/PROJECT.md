...
M049/S01, S02, S03, and S04 are now green: `meshc init --template todo-api --db postgres <name>` emits the migration-first shared/deployable starter from S01, `meshc init --template todo-api --db sqlite <name>` emits the explicit local single-node starter from S02, and S03 checks in generator-owned `/examples/todo-postgres` and `/examples/todo-sqlite` as public `meshc init` output instead of hand-maintained showcase trees. S04 then retired the repo-root `tiny-cluster/` and `cluster-proof/` onboarding surfaces entirely, moved those proofs under `scripts/fixtures/clustered/`, repointed retained Rust and shell rails to shared fixture helpers, and made the public clustered story scaffold/examples-first while keeping the lower-level proof fixtures available only through retained verifier/docs surfaces. The SQLite path still passes generator, tooling, and live runtime rails; `/health` reports local SQLite state without clustered metadata; restart persistence is proven against the retained `todo.sqlite3`; the old clustered SQLite Todo contract now lives behind the committed M047 fixture plus retained verification bundles instead of behind the public scaffold; and the `/examples` surface has a repo-owned materializer/check command plus retained parity/build/test bundles under `.tmp/m049-s03/`. The remaining M049 slice is the assembled reset verifier.
...
- broader public docs still need the M050 evaluator-facing rewrite, but the old top-level proof-app onboarding surfaces have been retired in favor of scaffold plus generated `/examples`; deeper/internal clustered proof rails now live under `scripts/fixtures/clustered/` and retained verifier-oriented docs
