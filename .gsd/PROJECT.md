...
The current wave has closed the evaluator-facing public-docs reset and moved into the reference-app retirement wave: M050 is complete, and M051 is active on its last slice (S05) after landing Mesher bootstrap modernization, retained backend-proof extraction, bounded tooling-fixture migration, and the public examples-first docs/scaffold/skill retarget. Later milestones still align landing/packages/deploy/load-balancing surfaces with that same truthful story.
...
M051 is active on S05. S01 refit Mesher onto the current bootstrap/runtime contract with a maintainer runbook and live Postgres rail, S02 extracted backend-only proof into retained fixtures plus `bash scripts/verify-m051-s02.sh`, S03 moved tooling/editor/LSP/formatter rails onto the bounded retained backend fixture, and S04 retargeted public docs, scaffold output, and clustering skills to the examples-first story while keeping Mesher maintainer-facing. The remaining work is S05: delete `reference-backend/` and close the post-deletion assembled acceptance rail.
...
- dogfooded applications: `mesher/` as the maintained deeper reference app, repo-root `reference-backend/` as a shrinking compatibility surface until M051/S05 deletes it, and retained clustered proof fixtures under `scripts/fixtures/clustered/tiny-cluster` plus `scripts/fixtures/clustered/cluster-proof`
...
- M050 is complete: the public docs graph, first-contact pages, and secondary proof surfaces now all route readers through scaffold/examples-first material with fast preflights via `bash scripts/verify-m050-s01.sh`, `bash scripts/verify-m050-s02.sh`, `bash scripts/verify-m050-s03.sh`, and the assembled `bash scripts/verify-m049-s05.sh`
- the landing site still reflects stale product positioning rather than Mesh's actual language story
- M051 is active on its last slice, so `reference-backend/` still exists only as a compatibility surface until the post-deletion closeout rail lands in S05
