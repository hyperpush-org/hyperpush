...
- [ ] **T03: Refresh the Mesh init-time skill bundle and pin clustered/runtime guidance** — Refresh the auto-loaded Mesh skill bundle so first-contact clustered guidance matches the current source-first runtime story. Add a dedicated clustering sub-skill, route the root skill plus syntax/http sub-skills through it, and add `scripts/tests/verify-m048-s04-skill-contract.test.mjs` so missing `@cluster`, `Node.start_from_env()`, scaffold commands, operator commands, or `HTTP.clustered(...)` guidance fails closed. Done when the skill contract test proves the cluster/runtime story is present across the exact files the init-time loader exposes.
  - Estimate: 2h
